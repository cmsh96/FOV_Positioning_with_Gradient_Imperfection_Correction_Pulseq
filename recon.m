% function sos = recon_fov_gd(path , n, ktraj_adc, t_adc, ktraj, t_ktraj, t_excitation, t_refocusing, original_seq)
    ...
    D=dir([path pattern]);
    [~,I]=sort([D(:).datenum]);
    seq_file_path = [path D(3).name]; % use end-1 to reconstruct the second-last data set, etc.
    ...
    % traj_recon_delay=[-10,-10,-10]*1e-6;
    grad_offsets=[0 0 0];
    ...
    [ktraj_adc, t_adc, ktraj, t_ktraj, t_excitation, t_refocusing, ~, ~, ~, pm_adc_nom] = seq.calculateKspacePP('trajectory_delay', traj_recon_delay, 'gradient_offset', grad_offsets);

    gw_data = cell(1,3);
    % predict the actual grad waveforms using the nominal grad waveforms and
    % the measured GIRF signal
    gw_data{1} = [G_sp64_25uscor(1,:);G_sp64_25uscor(2,:)]; %[G_sp64_iso(1,:);G_sp64_iso(2,:)];
    gw_data{2} = [G_sp64_25uscor(1,:);G_sp64_25uscor(3,:)]; %[G_sp64_iso(1,:);G_sp64_iso(3,:)];
    gw_data{3} = [G_sp64_25uscor(1,:);G_sp64_25uscor(4,:)]; %[G_sp64_iso(1,:);G_sp64_iso(4,:)];

    ext.gw_data        = gw_data;
    ext.t_adc          = t_adc;
    ext.tfp_excitation = [t_excitation ; zeros(size(t_excitation))];
    ext.tfp_refocusing = [t_refocusing ; zeros(size(t_refocusing))];
    ext.pm_adc         = pm_adc_nom;
    [ktraj_adc_girf, t_adc, ktraj_girf, t_ktraj, t_excitation, t_refocusing, slicepos, t_slicepos, gw_pp, pm_adc] = seq.calculateKspacePP( 'externalWaveformsAndTimes', ext, 'trajectory_delay', traj_recon_delay, 'gradient_offset', grad_offsets);

    ktraj_adc = real(ktraj_adc_girf); ktraj = real(ktraj_girf); 
    ...
    %% ** apply adc phase modulation to the data
    % for spiral

    % % **********************************************
    % % paper figure spiral with simulated 10us 
    % % first step
    % adc_counter = 0;
    ...
    %         if isempty(B.adc.phaseModulation)
    %             B.adc.phaseModulation = zeros(B.adc.numSamples,1);
    %         end
    %         aditional_adc_phase = exp(-1i*B.adc.phaseModulation);
    %         data_unsorted(:,:,(adc_counter-1)*2+1) = data_unsorted(:,:,(adc_counter-1)*2+1) .* repmat(aditional_adc_phase(1:6300),1,N_coils);
    %         data_unsorted(:,:,(adc_counter-1)*2+2) = data_unsorted(:,:,(adc_counter-1)*2+2) .* repmat(aditional_adc_phase(6301:12600),1,N_coils);
    %     end
    % end
    % % paper figure spiral with simulated 10us 
    % % second step
    ...
    %             B.adc.phaseModulation = zeros(B.adc.numSamples,1);
    %         end
    %         aditional_adc_phase = exp(-1i*B0.adc.phaseModulation);
    %         % data_unsorted = data_unsorted .* repmat(aditional_adc_phase,1,N_coils);
    %         data_unsorted(:,:,(adc_counter-1)*2+1) = data_unsorted(:,:,(adc_counter-1)*2+1) .* repmat(aditional_adc_phase(1:6300),1,N_coils);
    %         data_unsorted(:,:,(adc_counter-1)*2+2) = data_unsorted(:,:,(adc_counter-1)*2+2) .* repmat(aditional_adc_phase(6301:12600),1,N_coils);
    %     end
    % end
    ...
    % **********************************************
    % paper figure spiral (no simulated grad delay) 
    % first step
    % adc_counter = 0;
    % for iB=1:length(seq.blockEvents)
    %     B = seq.getBlock(iB);
    ...
    %         end
    %         aditional_adc_phase = exp(-1i*B.adc.phaseModulation);
    %         data_unsorted(:,:,adc_counter) = data_unsorted(:,:,adc_counter) .* repmat(aditional_adc_phase,1,N_coils);
    %     end
    % end
    % paper figure spiral with simulated 10us 
    % second step
    ...
    %         if ~isempty(B.adc)
    %             adc_counter = adc_counter+1;
    %             if isempty(B0.adc.phaseModulation)
    %                 B.adc.phaseModulation = zeros(320,1);
    %             end
    %             aditional_adc_phase = exp(-1i*B0.adc.phaseModulation);
    %             data_unsorted(:,:,adc_counter) = data_unsorted(:,:,adc_counter) .* repmat(aditional_adc_phase,1,N_coils);
    %         end
