...
% here the radial gre sequence has 360 full-radial spokes and the opposing spokes are used to estimate the grad delay 
path = '...\GradDelayEst'; % 

pattern='*.seq';
D=dir([path filesep pattern]);
[~,I]=sort([D(:).datenum]);
data_file_path=[path filesep D(1).name]; % use end-1 to reconstruct the second-last data set, etc...
